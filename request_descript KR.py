...
write_ws.append(["IMAGE","TIMESTAMP","DESCRIPT","SIMILARITYSCORE","SLEEPCHANGE", "SLEEPCHECK", "SLEEPPREDICTION", "SLEEP_GT", "SLEEP_GT_CHECK", "TOTAL_SCORE"])
...
while(video.isOpened()):
    ...
    if(frame % get_frame == 0):
        ...
        result = infer_from_server_with_image_object("http://172.16.8.52:8000", request_img, "아기가 잠을 자고 있나요? 아기가 깨어 있다면 무슨 행동을 하는 중인가요?","Llama3.2-VIX-M-3B-KO")
        end_time = time.time()
        ...
        write_ws['F'+str(count)] = sleep_descript

        #GT 체크
        
        for GT_DICT in GT_json:
            if GT_DICT["TIMESTAMP"] == Time:
                write_ws['H'+str(count)] = GT_DICT["IS_SLEEP"]
                if GT_DICT["IS_SLEEP"] == "O":
                    if sleep_descript == "아기가 자는 중입니다.":
                        write_ws['I'+str(count)] = "O"
                        GT_count += 1
                    else :
                        write_ws['I'+str(count)] = "X"
                else :
...
write_ws['J2'] = (GT_count / count) * 100
result_count = count
...
result_ws.append(["IMAGE","TIMESTAMP","DESCRIPT","SIMILARITYSCORE","SLEEPCHANGE", "SLEEPCHECK", "SLEEPPREDICTION", "SLEEP_GT", "SLEEP_GT_CHECK", "TOTAL_SCORE"])
#"아기 수면 여부", "수면 스코어"
cell_num = 1
count_img = -1
...
result_ws['J2'] = (result_GT_count / result_count) * 100
write_wb.save(filename="excel/description.xlsx")
code_end = time.time()
